...
def _guess_mariadb_version() -> tuple[int] | None:
        ...
        with suppress(Exception):
                ...
                version_output = subprocess.check_output([mariadb, "--version"], text=True, stderr=subprocess.DEVNULL)
...
def validate_database_sql(path: str, _raise: bool = True) -> None:
        ...
        if os.path.getsize(path):
                ...
                try:
                        frappe.utils.execute_in_shell([executable, "-m1", "__Auth", path], check_exit_code=True)
                        return
                except Exception:
