...
def get_info_via_oauth(provider: str, code: str, decoder: Callable | None = None, id_token: bool = False):
        ...
        if id_token:
                ...
                info = jwt.decode(token, flow.client_secret, options={"verify_signature": False})  # nosec B506 - signature verification skipped because providers may use RS256 with JWKS; token was received over HTTPS directly from the provider in the same session
