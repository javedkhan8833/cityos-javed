...
def get_assets_link(frappe_head) -> str:
        ...
        try:
                show_ref_output = subprocess.check_output(
                        ["git", "show-ref", "--tags", "-d"],
                        cwd="../apps/frappe",
                ...
                tag = ""
                for line in show_ref_output.strip().splitlines():
                        if frappe_head in line:
                                tag = re.sub(r".*\brefs/tags/", "", line)
                                tag = tag.replace("^{}", "")
                                break
...
def download_frappe_assets(verbose=True):
        ...
        """
        try:
                frappe_head = subprocess.check_output(
                        ["git", "rev-parse", "HEAD"], cwd="../apps/frappe", text=True, stderr=subprocess.DEVNULL
                ).strip()
        except subprocess.CalledProcessError:
                frappe_head = ""
...
def check_node_executable():
        warn = "⚠️ "
        try:
                node_version = Version(subprocess.check_output(["node", "-v"], text=True, stderr=subprocess.DEVNULL).strip()[1:])
        except Exception:
                click.echo(f"{warn} Node.js is not installed or not in PATH")
                return
