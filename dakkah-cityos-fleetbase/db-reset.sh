...
echo "Step 1: Patching vendor migrations..."
...
echo "  Patched duplicate uuid unique constraints"
...
MIGRATION_EOF
  echo "  Patched performance indexes migration"
fi
...
PAT_EOF
  echo "  Patched fix_personal_access_tokens migration (raw SQL instead of Doctrine)"
fi

echo ""
echo "Step 2-4: Wipe → Migrate → Seed (running as single PHP process)..."
php -d memory_limit=1024M -d max_execution_time=0 database/db-reset-runner.php 2>&1
