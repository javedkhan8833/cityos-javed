...
return new class extends Migration {
    ...
    public function up()
    {
        ...
        Schema::table('devices', function (Blueprint $table) {
            $table->string('public_id')->after('uuid')->nullable()->index();
            $table->string('company_uuid', 191)->nullable()->after('_key')->constrained('companies', 'uuid')->nullOnDelete();
            $table->string('photo_uuid', 191)->nullable()->after('telematic_uuid')->constrained('files', 'uuid')->nullOnDelete();
        });

        // Update sensors table
        Schema::table('sensors', function (Blueprint $table) {
            $table->string('public_id')->after('uuid')->nullable()->index();
            $table->string('photo_uuid', 191)->nullable()->after('company_uuid')->constrained('files', 'uuid')->nullOnDelete();
        });
